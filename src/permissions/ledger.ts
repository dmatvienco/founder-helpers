...
function saveLedger(projectRoot: string, ledger: Ledger): void {
  ...
  writeClaudeSettings(projectRoot, configOf(projectRoot), ledger);
}
 ...
 */
export function recordGrant(projectRoot: string, input: RecordGrantInput): Grant {
...
export function revokeGrant(projectRoot: string, id: string): Grant {
  const ledger = readLedger(projectRoot);
